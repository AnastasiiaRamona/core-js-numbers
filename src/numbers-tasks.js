...
function getSumOfDigits(num) {
  const arrNumber = String(num).split('');
  let count = 0;
  for (let i = 0; i < arrNumber.length; i += 1) {
    count += Number(arrNumber[i]);
  }
  return count;
}
...
function isPowerOfTwo(num) {
  return Math.log2(num) % 1 === 0 && num > 0;
}
...
function roundToLargestInteger(number) {
  const largestNumber = Math.ceil(number);
...
function getCountOfOddNumbers(number) {
  let count = 0;
  for (let item = 1; item <= Math.abs(number); item += 1) {
    if (item % 2 !== 0) {
      count += 1;
    }
  }
